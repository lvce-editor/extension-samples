...
test('invalid setup reports an error while keeping the source editable', async ({ page }) => {
  ...
  await page.goto('/extension-samples/completion-provider/')
  await expect(page.getByRole('status')).toContainText('Preview setup failed:', { timeout: 30_000 })
  await expect(page.getByRole('status')).toContainText('Export a setupPreview function')
