...
test('quick input starts with a default and writes the entered greeting', async ({ page }) => {
  ...
  await expect(page.locator('.QuickPickStatus')).toHaveCount(0)
  await expect(page.locator('.QuickPick [role="listbox"]')).toHaveCount(0)
  await expect(page.getByRole('textbox', { name: 'Who should we greet?' })).toBeVisible()
  await input.fill('')
  await expect(page.locator('.QuickPickStatus')).toHaveCount(0)
  await input.fill('  Ada  ')
  await expect(page.locator('.QuickPickStatus')).toHaveCount(0)
  await expect(page.locator('.QuickPickItem')).toHaveCount(0)
