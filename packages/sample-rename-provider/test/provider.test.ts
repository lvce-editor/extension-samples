...
await test('rename uses original offsets for a longer replacement without matching identifier prefixes', () => {
  ...
  let { text } = document
  const edits = changes.edits.toSorted((a, b) => b.offset - a.offset)
